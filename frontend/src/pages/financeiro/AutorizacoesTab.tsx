...
export default function AutorizacoesTab() {
    ...
    const { data: autorizacoes, isLoading, refetch } = useQuery({
        queryKey: ["autorizacoes", statusFilter],
        queryFn: async () => {
    ...
    const filtered = autorizacoes?.filter(a =>
        a.cliente.nome_fantasia?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        a.titulo_anuncio.toLowerCase().includes(searchTerm.toLowerCase()) ||
